...
var Calc = exports.Calc = function(build) {
	...
	/*
		Add the Gamedata to the Calculator
	*/
	if (typeof module !== 'undefined' && module.exports) {
		// If we're inside node.js, just require it
		this.gamedata = require("../data/gamedata.json");
	} else {
		// If we're in a browser, grab it out of the d3up namespace
		this.gamedata = window.d3up.gamedata;
	}
};
...
Calc.prototype.calcBase = function() {
	...
	// Apply our baseAttrs
	_.each(baseAttrs, function(value, attr) {
	...
	"switch": function(calc, data) {
		...
		_.each(data.cases, function(cCase) {
			if(_.indexOf(cCase.caseOf.split("|"), thisCase) >= 0) {
				_.each(cCase.effect, function(value, attr) {
					this.attr(attr, value);
				}, this);
			}
		}, calc);
...
Calc.prototype.parseSkills = function() {
	...
	for(active in this.build.actives) {
		...
		this.actives[skill] = _.cloneDeep(this.gamedata.actives[heroClass][skill]);
	}
	for(passive in this.build.passives) {
		skill = this.build.passives[passive],
		this.passives[skill] = _.cloneDeep(this.gamedata.passives[heroClass][skill]);
	}
	return this;
};
...
})(typeof exports === 'undefined'? this['d3up'] = {} : exports);